...
class NginxQuotedString(QuotedString):
    def __init__(self, quoteChar):
        super().__init__(quoteChar, escChar="\\", multiline=True)
...
class RawParser:
    ...
    def script(self):
        ...
        location_modifier = Keyword("=") | Keyword("~*") | Keyword("~") | Keyword("^~")
        ...
        condition_body = (if_modifier + Optional(space) + value) | (
        ...
        directive = (keyword + ZeroOrMore(space + value) + semicolon)("directive")

        file_delimiter = (Suppress("# configuration file ") + path + Suppress(":"))(
            "file_delimiter"
        )

        comment = (Regex(r"#.*"))("comment").setParseAction(_fix_comment)

        hash_value = Group(value + ZeroOrMore(space + value) + semicolon)("hash_value")
        ...
        if_block << (
            ...
            + Group(left_bracket + Optional(sub_block) + right_bracket)
        )("block")

        location_block << (
            Keyword("location")
            + Group(Optional(space + location_modifier) + Optional(space) + value)
            + Suppress(Optional(comment))
            + Group(left_bracket + Optional(sub_block) + right_bracket)
        )("block")
        ...
        hash_block << (
            ...
            + Group(left_bracket + Optional(OneOrMore(hash_value)) + right_bracket)
        )("block")
...
def _fix_comment(string, location, tokens):
    ...
    :param string: original parse string :param location: location in the string where
    matching started :param tokens: list of the matched tokens, packaged as a
    ParseResults_ object :return: list of the cleared comment tokens
    """
