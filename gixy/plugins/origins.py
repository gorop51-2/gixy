...
class origins(Plugin):
    ...
    directives = ["if"]
    options = {"domains": ["*"], "https_only": False}

    def __init__(self, config):
        super().__init__(config)
        if (
            self.config.get("domains")
            and self.config.get("domains")[0]
            and self.config.get("domains")[0] != "*"
        ):
        ...
        scheme = "https{http}".format(
            http=("?" if not self.config.get("https_only") else "")
        )
        regex = r"^{scheme}://(?:[^/.]*\.){{0,10}}(?P<domain>{domains})(?::\d*)?(?:/|\?|$)".format(
            scheme=scheme, domains=domains
        )
    ...
    def audit(self, directive):
        ...
        regexp = Regexp(
            directive.value, case_sensitive=(directive.operand in ["~", "!~"])
        )
        ...
        if invalid_referers:
            ...
            reason = 'Regex matches "{value}" as a valid {name}.'.format(
                value=invalid_referers, name=name
            )
