...
def test_from_config():
    ...
    for plugin in manager.plugins:
        ...
        assert_true(
            ...
            "Plugin {name!r} should have at least one simple test config".format(
                name=plugin
            ),
        )
        assert_true(
            plugin in tested_fp_plugins,
            "Plugin {name!r} should have at least one simple test config with false positive".format(
                name=plugin
            ),
        )
...
def yoda_provider(plugin, plugin_options=None):
    config = Config(allow_includes=False, plugins=[plugin])
    if plugin_options:
...
def check_configuration(plugin, config_path, test_config):
    ...
    with yoda_provider(plugin, plugin_options) as yoda:
        ...
        assert_true(
            result["help_url"].startswith("https://"),
